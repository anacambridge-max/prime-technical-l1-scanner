...
async function resolveInstruments(token: string): Promise<Instrument[]> {
  ...
  cachePromise = (async () => {
    const results = await inBatches(UNIVERSE, 10, async (symbol) => {
      ...
      url.searchParams.set("query", symbol); url.searchParams.set("exchanges", "NSE");
      url.searchParams.set("segments", "EQ"); url.searchParams.set("instrument_types", "EQ"); url.searchParams.set("records", "10");
...
function getWeekKey(d: Date) {
  ...
  const day = (x.getDay() + 6) % 7; x.setDate(x.getDate() - day + 3);
...
async function getLevels(token: string, instrument: Instrument, today: Date): Promise<Levels | null> {
  const cached = levelsCache.get(instrument.instrument_key);
  if (cached && Date.now() - cached.at < LEVEL_TTL) return cached.value;
  ...
  const ordered = [...days].sort((a, b) => new Date(a[0]).getTime() - new Date(b[0]).getTime());
  const pd = ordered[ordered.length - 1];
  const currentWeekKey = getWeekKey(today);
  const week = ordered.filter(c => getWeekKey(new Date(c[0])) === currentWeekKey);
  ...
  const y52 = ordered.filter(c => new Date(c[0]).getTime() >= yearAgo.getTime());
  const monthBars = months.length ? months : ordered;

  const value: Levels = {
    pdh: Number(pd[2]), pdl: Number(pd[3]),
    weeklyHigh: Math.max(...(week.length ? week : [pd]).map(c => Number(c[2]))),
    weeklyLow: Math.min(...(week.length ? week : [pd]).map(c => Number(c[3]))),
    monthlyHigh: Math.max(...(month.length ? month : [pd]).map(c => Number(c[2]))),
    monthlyLow: Math.min(...(month.length ? month : [pd]).map(c => Number(c[3]))),
    high52w: Math.max(...(y52.length ? y52 : [pd]).map(c => Number(c[2]))),
    low52w: Math.min(...(y52.length ? y52 : [pd]).map(c => Number(c[3]))),
    ath: Math.max(...monthBars.map(c => Number(c[2]))),
    atl: Math.min(...monthBars.map(c => Number(c[3]))),
  };
  levelsCache.set(instrument.instrument_key, { at: Date.now(), value });
  return value;
}
...
function score(rvol: number, breakout: string, master: boolean, opening: boolean) {
  let s = 40;
  if (rvol >= 3) s += 30; else if (rvol >= 2) s += 22; else if (rvol >= 1.5) s += 10;
  if (breakout === "ATH" || breakout === "ATL") s += 15;
  else if (breakout.includes("52W")) s += 12;
  else if (breakout.includes("WEEKLY") || breakout.includes("MONTHLY")) s += 8;
  else if (breakout === "PDH" || breakout === "PDL") s += 6;
  if (master) s += 8;
  if (opening) s += 7;
...
export async function GET(request: NextRequest) {
  ...
  const rows = await inBatches(instruments, 8, async (instrument): Promise<{ signal: Row | null; breakout: Row | null; extreme: Row | null; candidate: Row | null }> => {
    const cs = await intraday(token, instrument.instrument_key);
    const closed = completedCandles(cs, d);
    if (closed.length < 2) return { signal: null, breakout: null, extreme: null, candidate: null };
    const levels = await getLevels(token, instrument, d);
    if (!levels) return { signal: null, breakout: null, extreme: null, candidate: null };
    ...
    const prior = closed.slice(0, -1).slice(-5);
    ...
    const currentRange = Math.max(0, Number(current[2]) - Number(current[3]));
    const priorAvgRange = prior.length ? prior.reduce((s, c) => s + (Number(c[2]) - Number(c[3])), 0) / prior.length : currentRange;
    const master = currentRange >= priorAvgRange * 1.5;
    const openingBreak = !!opening && (current[4] > opening[2] || current[4] < opening[3]);
    const kb = keyBreakout(current, levels);
    const extreme = rvol >= 2;
    const candleSide = current[4] >= current[1] ? "BUY" as const : "SELL" as const;
    ...
    const extremeRow = extreme ? { symbol: instrument.trading_symbol, setup: master ? "MASTER CANDLE" : openingBreak ? "OPENING CANDLE" : "EXTREME VOLUME", breakout: kb?.name ?? "NONE", volume: rvol >= 3 ? "EXTREME" : "2X+", rvol: Number(rvol.toFixed(2)), price: Number(current[4]), entry: Number(current[4]), sl: Number(current[3]), slPct: Number((Math.abs(Number(current[4]) - Number(current[3])) / Number(current[4]) * 100).toFixed(2)), target2R: Number(current[4]) + Math.abs(Number(current[4]) - Number(current[3])) * 2, score: score(rvol, kb?.name ?? "NONE", master, openingBreak), side: candleSide, candleTime: current[0] } as Row : null;

    let signal: Row | null = null;
    // User rule: new master/opening/key-level BUY/SELL signals only before 10:00, and always require extreme volume.
  ...
  const signals = rows.map(x => x.signal).filter(Boolean) as Row[];
  const breakouts = rows.map(x => x.breakout).filter(Boolean) as Row[];
  const extremeVolume = rows.map(x => x.extreme).filter(Boolean) as Row[];
  const candidates = rows.map(x => x.candidate).filter(Boolean) as Row[];
  ...
  const buys = signals.filter(x => x.side === "BUY").length, sells = signals.filter(x => x.side === "SELL").length;
  const avgScore = signals.length ? Math.round(signals.reduce((s, x) => s + x.score, 0) / signals.length) : 0;

  return NextResponse.json({
    connected: true, marketOpen: true, signalWindow, marketTime, scanned: instruments.length,
    signals, breakouts, extremeVolume, candidates,
    stats: { buy: buys, sell: sells, extreme: extremeVolume.length, avgScore, setups: signals.length },
    rule: "BUY/SELL confirmation: 5M candle + Extreme Volume (>=2x prior 5 closed candles) + breakout of PDH/PDL, Weekly H/L, Monthly H/L, 52W H/L or ATH/ATL. Master/Opening signals are only generated before 10:00 IST. Breakout candidates remain visible after 10:00.",
  });
}
